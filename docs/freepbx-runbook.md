...
## 8. ブラウザ (UCP) から ConfBridge に参加する

### 前提

- FreePBX の UCP にログインするユーザー (Extension) が作成済みであること

### 手順

**1. WebRTC モジュールの確認**

Admin → Module Admin → `webrtc` を検索 → インストール済みで Enabled であることを確認

**2. Extension に WebRTC を有効化**

Connectivity → Extensions → 対象 Extension → Advanced タブ → **Enable WebRTC defaults: Yes** → Submit → Apply Config

**3. UCP ユーザーへの権限付与**

Admin → User Management → 対象ユーザー → UCP タブ → **Phone: Yes** → Save

**4. Asterisk HTTPS Bind Address の変更**

Settings → Advanced Settings → Asterisk Builtin mini-HTTP server セクション → **HTTPS Bind Address: `0.0.0.0`** → Submit → Apply Config

確認 (Admin → Asterisk CLI):

```
http show status
# HTTPS Server Enabled and Bound to 0.0.0.0:8089 になっていること
```

**5. ブラウザで証明書を受け入れる**

Chrome で `https://<PBX-IP>:8089/` にアクセス → 詳細設定 → サイトにアクセス → `Not Found` と表示されれば証明書の受け入れ完了

**6. UCP で Phone ウィジェットを追加**

`https://<PBX-IP>/ucp` にログイン → ウィジェット追加 → Phone → `Registered` になることを確認

**7. ConfBridge に接続**

Phone ウィジェットから **8000** にダイヤル
